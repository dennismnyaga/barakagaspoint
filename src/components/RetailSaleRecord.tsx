...
const RetailSaleRecord = () => {
  ...
  // const selecte = allAssignedProducts.sales_team.products.find((product) => product.id === selectedProduct)
  const userSalesTeam = salesteam.find((sales) =>
  // @ts-ignore
    sales.employees.some((employee) => user?.id === employee.id),
  )
  const assignedCylindersForUserSalesTeam = allAssignedProducts.filter(
    (assignedCylinder) => assignedCylinder.sales_team.id === userSalesTeam?.id,
  )
  const calculateDebt = () => {
    if (deposit <= 0) {
      return 0
  ...
  useEffect(() => {
    const calculatedDebt = calculateDebt() // Calculate the debt value
    // @ts-ignore
    setDebt(calculatedDebt) // Update the debt state with the calculated value
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (response.status === 201) {
        // Handle a successful response (status code 201 Created)
  ...
  return (
    ...
    <option key={assignedProduct.cylinder.id} value={assignedProduct.cylinder.id}>
      {assignedProduct.cylinder.gas_type.name} {assignedProduct.cylinder.weight.weight}(Kg)
                ...
                {assignedCylindersForUserSalesTeam.map((assignedProduct) => (
                  <option
                    key={assignedProduct.cylinder.id}
                    value={assignedProduct.id}
                  >
                    {assignedProduct.cylinder.gas_type.name}{" "}
                    {assignedProduct.cylinder.weight.weight}(Kg)
                  </option>
                ))}
              </select>
...
export default RetailSaleRecord
